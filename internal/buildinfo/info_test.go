...
func TestInfoProvidesStableShortAndVerboseOutput(t *testing.T) {
	...
	if info.Short() != "repogate 1.2.3 (build 1.2.3-0123456789ab)" || info.BuildID != "1.2.3-0123456789ab" {
		t.Fatalf("unexpected build information: %#v", info)
	}
	for _, expected := range []string{"RepoGate Version: 1.2.3", "Git Commit: 0123456789abcdef", "Target Architecture:", "Build ID: 1.2.3-0123456789ab"} {
