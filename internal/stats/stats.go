...
func (s *Stats) Metrics(w http.ResponseWriter, names map[int64]string) {
	...
	fmt.Fprintf(w, "# TYPE repogate_active_requests gauge\nrepogate_active_requests %d\n", snap.Active)
	fmt.Fprintf(w, "# TYPE repogate_requests_total counter\nrepogate_requests_total %d\n", snap.Total.Requests)
	fmt.Fprintf(w, "# TYPE repogate_bytes_total counter\nrepogate_bytes_total %d\n", snap.Total.Bytes)
	fmt.Fprintf(w, "# TYPE repogate_upstream_bytes_total counter\nrepogate_upstream_bytes_total %d\n", snap.Total.UpstreamBytes)
	fmt.Fprintf(w, "# TYPE repogate_cache_bytes_total counter\nrepogate_cache_bytes_total %d\n", snap.Total.CacheBytes)
	fmt.Fprintf(w, "# TYPE repogate_upstream_errors_total counter\nrepogate_upstream_errors_total %d\n", snap.Total.UpstreamErrors)
	fmt.Fprintf(w, "# TYPE repogate_cache_hit_total counter\nrepogate_cache_hit_total %d\n", snap.Total.CacheHits)
	fmt.Fprintf(w, "# TYPE repogate_cache_miss_total counter\nrepogate_cache_miss_total %d\n", snap.Total.CacheMisses)
	fmt.Fprintf(w, "# TYPE repogate_go_heap_alloc_bytes gauge\nrepogate_go_heap_alloc_bytes %d\n", snap.Runtime.HeapAllocBytes)
	fmt.Fprintf(w, "# TYPE repogate_go_allocated_bytes_total counter\nrepogate_go_allocated_bytes_total %d\n", snap.Runtime.TotalAllocBytes)
	fmt.Fprintf(w, "# TYPE repogate_go_mallocs_total counter\nrepogate_go_mallocs_total %d\n", snap.Runtime.Mallocs)
	fmt.Fprintf(w, "# TYPE repogate_go_frees_total counter\nrepogate_go_frees_total %d\n", snap.Runtime.Frees)
	fmt.Fprintf(w, "# TYPE repogate_process_resident_memory_bytes gauge\nrepogate_process_resident_memory_bytes %d\n", snap.Runtime.RSSBytes)
	fmt.Fprintf(w, "# TYPE repogate_go_goroutines gauge\nrepogate_go_goroutines %d\n", snap.Runtime.Goroutines)
	fmt.Fprintf(w, "# TYPE repogate_process_open_fds gauge\nrepogate_process_open_fds %d\n", snap.Runtime.OpenFDs)
	fmt.Fprintf(w, "# TYPE repogate_go_gc_cycles_total counter\nrepogate_go_gc_cycles_total %d\n", snap.Runtime.GCCount)
	fmt.Fprintf(w, "# TYPE repogate_go_gc_pause_seconds_total counter\nrepogate_go_gc_pause_seconds_total %g\n", float64(snap.Runtime.GCPauseTotalNS)/float64(time.Second))
	fmt.Fprintf(w, "# TYPE repogate_go_gc_cpu_fraction gauge\nrepogate_go_gc_cpu_fraction %g\n", snap.Runtime.GCCPUFraction)
	...
	for _, id := range ids {
		...
		fmt.Fprintf(w, "repogate_requests_today{%s} %d\nrepogate_bytes_today{%s} %d\n", labels, c.Requests, labels, c.Bytes)
		fmt.Fprintf(w, "repogate_cache_hits_today{%s} %d\nrepogate_cache_misses_today{%s} %d\n", labels, c.CacheHits, labels, c.CacheMisses)
		fmt.Fprintf(w, "repogate_upstream_errors_today{%s} %d\n", labels, c.UpstreamErrors)
		fmt.Fprintf(w, "repogate_http_responses_today{%s,class=\"2xx\"} %d\n", labels, c.Status2xx)
		fmt.Fprintf(w, "repogate_http_responses_today{%s,class=\"3xx\"} %d\n", labels, c.Status3xx)
		fmt.Fprintf(w, "repogate_http_responses_today{%s,class=\"4xx\"} %d\n", labels, c.Status4xx)
		fmt.Fprintf(w, "repogate_http_responses_today{%s,class=\"5xx\"} %d\n", labels, c.Status5xx)
	}
}
