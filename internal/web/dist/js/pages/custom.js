...
export function initCustom() {
  ...
  $('#custom-form')?.addEventListener('submit', async event => {
    ...
    const body = {name: $('#custom-name').value, context: $('#custom-context').value, repository_id: Number($('#custom-repository').value), enabled: $('#custom-enabled').checked, content: $('#custom-content').value};
    try { await api(id ? `/custom-configs/${id}` : '/custom-configs', {method: id ? 'PUT' : 'POST', body: JSON.stringify(body)}); $('#custom-dialog').close(); notice(L('Custom configuration validated and activated.')); await loadCustom(); } catch (error) { $('#custom-error').textContent = error.message; }
  });
}
...
registerAction('delete-custom', async button => {
  ...
  try { await api(`/custom-configs/${Number(button.dataset.id)}`, {method: 'DELETE'}); notice(L('Custom configuration deleted.')); await loadCustom(); } catch (error) { notice(error.message, true); }
});
