...
func (g *Generator) mainConfig(customHTTP string) string {
    ...
    server {
        ...
        ssl_protocols %s;
        location / {
            proxy_pass http://repogate_frontend;
...
`, frontendServer, nginxListen(g.cfg.HTTP.Listen), nginxListen(g.cfg.HTTP.HTTPSListen),
	...
	return fmt.Sprintf(`# Generated by RepoGate. Manual edits are overwritten.
%sworker_processes %s;
pid %s;
...
http {
    ...
    proxy_cache_path %s levels=1:2 keys_zone=repogate_cache:64m inactive=%s max_size=%d min_free=%d use_temp_path=off;
    log_format repogate '$time_iso8601 request_id=$http_x_mirror_internal_request_id client_ip=$http_x_mirror_internal_client_ip repo=$http_x_mirror_internal_repository_id host=$host method=$request_method uri="$request_uri" status=$status bytes=$body_bytes_sent request_time=$request_time upstream="$upstream_addr" upstream_status="$upstream_status" upstream_time="$upstream_response_time" cache=$upstream_cache_status';
    access_log %s/access.log repogate;
...
%s
    ...
    server {
        listen %s;
        server_name repogate-upstream-nginx-internal;
        access_log %s/access.log repogate;
...
func upstreamGroupName(repository model.Mirror, upstream model.Upstream) string {
	return "repogate_repo_" + strconv.FormatInt(repository.ID, 10) + "_" + strconv.FormatInt(upstream.ID, 10)
}
...
func (g *Generator) writeCache(out *strings.Builder, repository model.Mirror, class string, ttl time.Duration) {
	...
	out.WriteString("            proxy_cache repogate_cache;\n")
...
func (g *Generator) integrationSnippet(repositories []model.Mirror) string {
	...
	out.WriteString("# External Shared Nginx integration generated by RepoGate.\n")
	out.WriteString("# Review certificate paths and include the required blocks manually.\n")
	out.WriteString("# RepoGate does not include this file or reload External Shared Nginx.\n\n")
	...
	if hasSharedAuxiliaryRoute {
		...
		out.WriteString(integrationLocation("/_repogate/upstream/", nginxProxyPass(g.cfg.FrontendEndpoint()), ""))
		out.WriteByte('\n')
	}
	if len(paths) > 0 {
...
var reservedCustomTokens = []string{"$repo_", "$health_", "$http_x_mirror_internal_", "x-mirror-internal-", "repogate_repo_", "repogate_cache", "repogate_frontend"}
