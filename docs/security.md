...
6. **Untrusted Origin Control Headers**: Every data-plane origin location ignores `X-Accel-Redirect`, `X-Accel-Expires`, `X-Accel-Limit-Rate`, `X-Accel-Buffering` and `X-Accel-Charset`. Only the Go authorization response may instruct the external ingress to accelerate a request; an origin cannot internally jump to another repository or package.
7. **Browser-Origin Isolation**: Every repository response carries an enforced CSP sandbox without `allow-same-origin`, plus `X-Content-Type-Options: nosniff`. This applies to HTML, SVG, rewritten documents and zero-copy downloads, independently of MIME type and `?safe-ui=1`. DOM scripts and downloads remain permitted, but fetches, workers, forms, plugins and embedding are blocked; upstream pages cannot access administration-origin storage or read administration responses. Production deployments should additionally configure a dedicated `admin.host` and serve no repository content on that administration origin. Do not remove the sandbox at an outer proxy or add `allow-same-origin` to it. See the [CSP sandbox semantics](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy/sandbox) and [Nginx origin-header processing](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_ignore_headers).
...
- **Revocation-Safe Renewal**: Sliding expiration updates only an existing, unexpired session and rejects a failed or zero-row refresh. Logout, account-wide revocation and password recovery cannot be undone by a stale request inserting the session again.
...
- Policy checks use decoded target paths for direct, `__fetch`, `__fetch_template` and signed auxiliary routes, and run again before every followed redirect. Targets beneath an upstream base (including `add_prefix`) use repository-relative names; other allowed origins use origin-relative names. Overlapping upstream bases must all satisfy the policy. Authentication-token exchange is not a package download and is exempt from package-name rules, but retains URL, origin and path validation.
- Local rewritten-metadata `304` responses are restricted to fresh public representations: cache-disabled or credentialed requests always reach the data plane, even with authenticated body caching enabled. Request/response cache directives, response cookies and unsupported `Vary` dimensions disable local reuse. Validator identities include content negotiation and the body-cache generation, so global, repository and object purges invalidate them, including older responses completing after a purge.
