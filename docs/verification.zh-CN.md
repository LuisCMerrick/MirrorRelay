...
make vendor-source VERSION=0.0.22 RELEASE_DIR=/tmp/mirrorrelay-source
tar -tzf /tmp/mirrorrelay-source/mirrorrelay-0.0.22-source-with-vendor.tar.gz \
  | grep -F 'mirrorrelay-0.0.22-source/vendor/modules.txt'
```
