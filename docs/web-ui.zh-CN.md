...
2. 打开 **Managed Upstream Nginx → 技术详情**，确认其版本、Build ID 和架构。
...
- **标准导出**（默认）：自动剔除敏感凭据（`distributed.token`、`distributed.mutation_token`、Webhook URL 与签名密钥、边缘节点密钥）、Passkey RP ID/Origins 以及本地实例公开访问地址（`http.public_base_url`、`distributed.node.public_base_url`）。
- **完整备份导出**：通过受 CSRF 保护的显式操作生成，包含凭据与 Passkey 绑定以供灾难恢复。本地实例公开 URL 仍会省略；下载文件必须按密钥材料保护。

### 配置导入

**导入配置** 功能支持上传 `.yaml` 文件或直接粘贴最多 1 MiB 的解码后 YAML 文本：
1. **启动级严格校验**：校验 YAML 语法、字段规范、取值范围与依赖关系。
2. **差异预览 (Diff)**：以表格形式清晰展示变更字段（`路径`、`当前值`、`导入值`），并提示是否需要重启生效。
3. **本地实例与敏感密钥保护**：若导入的 YAML 中省略了本地实例地址或密钥，自动保留当前运行实例的既有值。仅当 Edge 节点 URL 未改变时才会保留其 Mutation Token；把节点迁移到其他 Origin 时必须显式提供新 Token。
4. **确认应用**：原子持久化候选配置与脱敏历史记录；数据库/密钥环启动路径始终只允许文件配置。
5. **外观发布**：导入的 `ui_enhancement` 状态包含在同一事务中并立即发布；其余运行配置仍需按提示重启。

### 配置版本历史与回滚

通过设置页保存、导入、运行配置重置或回滚执行的每次变更均会记录至有界版本历史：
- 记录版本号、变更时间、操作人、来源（`web_ui`、`configuration_import`、`settings_rollback`、`settings_reset`）、Diff 差异摘要与安全快照。
- 历史快照绝不以明文保存敏感凭据或 Webhook 目标，历史 API 也不返回快照本身。
- 管理员可查看保留期内的版本并点击 **回滚**；脱敏字段会保留当前凭据，不会清空或恢复旧密钥。
- 回滚会立即恢复外观快照；需要重启的运行配置仍保持待应用状态，直至进程重启。
...
- **自定义 CSS**：启用通过 `/ui/custom.css` 提供的常规 `.css` 样式表；符号链接及大于 1 MiB 的文件会被拒绝。
