...
v0.0.22 requires no database migration. After upgrading, regenerate and review the External Shared Nginx integration snippet and activate the newly generated Managed Upstream Nginx configuration so the hardened internal repository location and response-header policy take effect. Repository HTML and SVG are now delivered in an opaque-origin sandbox; deployments that intentionally relied on active repository content sharing the administration origin must move that content to a separate trusted origin instead of relaxing the sandbox.
